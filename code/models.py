...
class ModelForCLM(torch.nn.Module):
    ...
    def forward(self, input_ids: torch.tensor, attention_mask: torch.tensor, labels: Union[None, torch.tensor]) -> torch.tensor:
        ...
        if labels is not None:
            labels = labels.to(self.device)  
            loss = self.loss_fn(z.flatten(0, 1), labels.flatten()) 
            acc = (z.argmax(dim=-1) == labels).to(torch.float32).mean()
        else:
            loss = None
            acc = None
        return {"logits": z, "loss": loss, "acc": acc.item()}
...
class ModelForCLMWithLoRA(torch.nn.Module):
    def __init__(self, device: torch.device, tokenizer: AutoTokenizer, model_name: str, num_class: int, lora_rank: int, lora_alpha: float, quant_config: Union[None, BitsAndBytesConfig]):
        super(ModelForCLMWithLoRA, self).__init__()
        self.model_name = model_name
        self.device = device
        self.tokenizer = tokenizer
        self.num_class = num_class
        self.model = ModelForCLM(device=self.device, model_name=self.model_name, num_class=self.num_class, quant_config=quant_config)
...
def main_clm(model_name: str, device: torch.device) -> None:
    ...
    model = ModelForCLMWithLoRA(device=device, tokenizer=tokenizer, model_name=model_name, num_class=3, lora_rank=8, lora_alpha=16, quant_config=quant_config).to(device)
    input_ids = torch.randint(low=0, high=100, size=(16, 256)).to(device) # (b, T)
    labels = torch.randint(low=0, high=3, size=(16, 256)).to(device) # (b,)
    attention_mask = torch.ones(size=(16, 256)).to(device) # (b, T)
    print(model)
...
if __name__ == "__main__":
    ...
    main_clm("meta-llama/Meta-Llama-3.1-8B", device=device)
