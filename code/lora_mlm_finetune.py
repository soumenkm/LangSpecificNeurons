...
from dataset import WikipediaDatasetHF, XNLIDatasetHF
...
class LoRAFineTuner:
    def __init__(self, device: torch.device, config: dict):
        ...
        self.ds = WikipediaDatasetHF(model_name=self.model_name, lang=self.lang, max_context_len=self.config["max_context_length"])
        ...
        self.model = ModelForCLMWithLoRA(device=self.device, tokenizer=self.tokenizer, model_name=self.model_name, num_class=self.config["num_class"], lora_rank=self.config["lora_rank"], lora_alpha=self.config["lora_alpha"], quant_config=self.quant_config)
...
def main(model_name: str, device: torch.device) -> None:
    config = {
        "model_name": model_name, "task_name": "Wiki-LoRA-FT",
        "lang": "en",
        "num_epochs": 2, "batch_size": 8, "max_context_length": 512, # steps are auto calculated
        "frac": 0.99, 
