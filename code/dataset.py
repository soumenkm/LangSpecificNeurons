...
class XQADatasetHF(Dataset):
    def __init__(self, model_name: str, lang: str, max_context_len: int, frac: float) -> None:
    ...
    def get_alpaca_formatted_input(self, example: dict) -> str:
        ...
        answer_text = self.tokenizer.sep_token + example["answers"]["text"][0].strip() + self.tokenizer.eos_token
    ...
    def get_dataset(self) -> Dataset:
        ...
        with tqdm.tqdm(iterable=range(len(dsl)), desc="Preparing dataset...", unit="example", colour="green") as pbar:
            for index in pbar:
                ...
                outputs = self.tokenizer(inputs, padding="max_length", truncation=True, max_length=512, return_tensors="pt") # (Tmax,)
...
def main_xqad(model_name: str):
    ds = XQADatasetHF(model_name=model_name, lang="en", max_context_len=512, frac=1.0)
    print(len(ds))
    print("DONE")
